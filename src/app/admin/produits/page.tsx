...
export default async function ProductsPage() {
  ...
  const [{ data: products }, { data: collections }] = await Promise.all([
    supabase.from('products').select('id, name_fr, slug, description_fr, base_price, currency_code, is_active, is_featured, is_new, product_images(id, url, alt_fr, is_primary, sort_order), product_variants(id, sku, size_label, color_name_fr, price, stock_quantity, low_stock_threshold)').is('deleted_at', null).order('sort_order'),
  ...
  return <div className="admin-page">
    ...
    <section className="admin-panel create-product-panel">
      ...
      <form action={createProduct} className="product-edit-form"><label>Nom de l’article<input name="name_fr" required placeholder="Pyjama satin rose"/></label><label>Prix de base FCFA<input name="base_price" type="number" min="0" required placeholder="25000"/></label><label className="field-wide">Description courte<textarea name="description_fr" rows={2}/></label><fieldset className="field-wide collection-products"><legend>Ajouter aux collections</legend>{collections?.map(collection => <label key={collection.id}><input type="checkbox" name="collection_ids" value={collection.id}/><span>{collection.name_fr}</span></label>)}</fieldset><fieldset className="field-wide collection-products"><legend>Tailles et prix</legend>{['S','M','L','XL','XXL','3XL'].map(size => <label key={size}><input type="checkbox" name="sizes" value={size}/><span>{size}</span><input name={`price_${size}`} type="number" min="0" placeholder="Prix FCFA"/></label>)}</fieldset><button className="admin-primary-button">Créer l’article</button></form>
    </section>
